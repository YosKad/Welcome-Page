...
loginForm.addEventListener("submit", function (event) {
  ...
  if (username === defaultUsername && password === defaultPassword) {
    ...
    document.getElementById(
      "attempt"
    ).innerHTML = `Hello ${username} you have logged in successfully`;
  } else {
    swal({
...
function bazBtn() {
  ...
  if (number % 2 === 0 && number % 3 === 0) {
    result.innerHTML = "Baz " + number;
  } else if (number % 2 === 0) {
    result.innerHTML = "Bar " + number;
  } else if (number % 3 === 0) {
  result.innerHTML = "Foo " + number;
  } else {
    result.innerHTML =
      "Not Divide By Any of the Number (2,3 or 2&3) " + number;
  }
}
